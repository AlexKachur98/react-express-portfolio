...
export default function GuestbookBoard() {
    ...
    const handleSubmit = async (event) => {
        ...
        setEntries((prev) => {
            const filtered = prev.filter((entry) => entry._id !== res._id && (typeof entry.user === 'object' ? entry.user?._id : entry.user) !== user?._id);
        ...
        <div className="contact-grid" style={{ width: '100%' }}>
            <div className="contact-grid__info">
                <p style={{ color: 'rgba(226,232,240,0.82)' }}>
                    Sign the guest book to leave a note. You can update or remove your entry anytime.
                </p>
                <div className="contact-grid__card">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '10px' }}>
                        ...
                        {loading && <span style={{ color: 'rgba(148,163,184,0.9)', fontSize: '0.95rem' }}>Loading...</span>}
                    </div>
                    ...
                    {entries.map((entry) => (
                        <div key={entry._id} style={{ borderBottom: '1px solid rgba(148,163,184,0.2)', padding: '10px 0' }}>
                            ...
                            {myEntry?._id === entry._id && (
                                <div style={{ marginTop: '6px', fontSize: '0.9rem', color: 'rgba(96,165,250,0.85)' }}>
                                    This is your note.
                                </div>
                            )}
                        </div>
            ...
            <div className="contact-grid__form">
                ...
                <h3 style={{ margin: '4px 0 12px' }}>{myEntry ? 'Update your note' : 'Leave a note in the guest book'}</h3>
                <form className="contact-form" onSubmit={handleSubmit}>
                    ...
                    <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
                        <button className="btn contact-form__submit" type="submit" disabled={saving}>
                            {saving ? 'Saving...' : myEntry ? 'Update entry' : 'Sign guest book'}
                        </button>
                        {myEntry && (
                            <button type="button" className="btn btn--ghost" onClick={handleDeleteMine}>
                                Remove my entry
                            </button>
                        )}
                    </div>
